...
class UpdateReportResultScreen extends StatefulWidget {
  const UpdateReportResultScreen(
  ...
  @override
  State<UpdateReportResultScreen> createState() => _UpdateReportResultScreenState();
}

class _UpdateReportResultScreenState extends State<UpdateReportResultScreen> {
  ...
  void initState() {
    ...
    tableRowDesignVm.getTableRowDesignItem(
        widget.patientService.id, widget.patientService.itemId,widget.patientService.groupItemIds).then((value)  {
      designList = value.design??[];

      listController = List.generate(
          value.design?.length??0,
              (index) => TextEditingController(text: designList[index].result));
